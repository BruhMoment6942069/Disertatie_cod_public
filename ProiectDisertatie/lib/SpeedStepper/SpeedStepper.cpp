...
SpeedStepper::SpeedStepper(int stepPin, int dirPin) :  maxMaxSpeed(1000.0), minMaxSpeed(0.0003), minPulseWidth(2) {
  DIR_PIN = dirPin;
  STEP_PIN = stepPin;
  // Configure pins as outputs BEFORE any digitalWrite. hardStop()/setAcceleration()
  // below drive DIR_PIN/STEP_PIN, and since these objects are usually constructed
  // globally (before setup()), writing an unconfigured GPIO triggers the ESP32
  // "IO N is not set as GPIO" error.
...
/**
   updates currentPosition for oneStep
   called from oneStep, Depends on direction
*/
void SpeedStepper::updateCurrentPosition() {
    if (isDirForward()) {
      // Clockwise
...
boolean SpeedStepper::runSpeed() {
  ...
  if (time - lastStepTime >= stepInterval) {
    oneStep(); // move current pos into oneStep
    lastStepTime = time;
    return true;
  }  else    {
    return false;
