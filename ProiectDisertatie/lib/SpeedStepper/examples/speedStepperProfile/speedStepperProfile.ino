...
// on the Arduino IDE Serial Plotter
...
pfodBufferedStream bufferedStream(SERIAL_OUT_BAUD_RATE, outputBuffer, OUTPUT_BUFFER_SIZE); // do not block just drop chars if buffere full
...
void setup() {
  ...
  serialIO = bufferedStream.connect(&SERIAL_OUT);
  // ------------------------------
